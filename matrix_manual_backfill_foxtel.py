...
def create_new_dynamic_dag(dag_id: str, client_name: str, client_ids: str, start_date: str, end_date: str):
    @dag(
        dag_id=f"{dag_id}_backfill_v3",  # New versioned DAG ID
        default_args={**default_args, 'start_date': datetime.strptime(start_date, '%Y-%m-%d'), 'end_date': datetime.strptime(end_date, '%Y-%m-%d')},
        description=f'Backfill extraction for {client_name} with BigLake integration.',
...
for client_config in config_v3.export_list:
    if client_config['client_name'] == 'foxtel':
        ...
        end_date = '2022-12-31'  # Define end_date for backfill

        # Create the new dynamic DAG (inside the loop!)
        globals()[f"{dag_id}_manual_backfill"] = create_new_dynamic_dag(dag_id, client_name, client_ids, start_date, end_date)
