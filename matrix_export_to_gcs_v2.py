...
import matrix_config_v2

# Load v2 configuration (isolated from production)
bq = matrix_config_v2.BigQueryConfig()
matrix = matrix_config_v2.MatrixConfig()
compute = matrix_config_v2.ComputeConfig()
storage = matrix_config_v2.StorageConfig()

PROJECT_ID = bq.project_id
# Use test bucket from v2 configuration
...
# Copy v2 scripts from GCS
setup_command = (
    "gsutil -m cp -r \"gs://au-accel-mdw-dev-scripts/matrix_export_v2/*\" .scripts_v2/"
)
...
def create_new_dynamic_dag(dag_id: str, client_name: str, client_ids: str, start_date: str):
    @dag(
        ...
        max_active_runs=1,
        tags=['matrix', 'v2', 'monthly', 'biglake'],
        doc_md=f"""
        # Monthly Matrix Extraction DAG v2 - {client_name}
    ...
    def taskflow():
        ...
        def extract():
            ...
            def run_extraction(**context):
                ...
                final_command = f"""cmd.exe /c python .scripts_v2\\main.py 
                    --project-id={PROJECT_ID} 
                    --destination-bucket={TEST_BUCKET} 
...
config_v2 = matrix_config_v2.Config()

for client_config in config_v2.export_list:
